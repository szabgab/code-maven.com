pipeline {
    ...
    stages {
        stage('Check disk usage') {
            steps {
                script {
                    manager.addShortText(params.text, params.foreground_color, params.background_color, params.border_size, params.border_color)
                    currentBuild.description = "Foreground: ${foreground_color}<br>Background: ${background_color}<br>Border size: ${params.border_size}<br>Border color: ${params.border_color} "
